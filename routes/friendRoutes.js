...
router.get('/filter', (req, res) => {
    ...
    if (filterLetter) {
        matchingFriends = matchingFriends.filter(friend => friend.name.charAt(0).toLowerCase() == filterLetter.toLowerCase());
    } else {
        res.status(404).json({ error: "No matching friend" });
    }
}
);
...
router.put('/:id', (req, res) => {
    ...
    if (index !== -1) {
        ...
        res.json({ result: 'Updated friend id ' + friendId, data: updatedFriend });
    } else {
        res.status(404).json({ error: 'Friend not found' });
    }
});
